...
class CauseChecker: 
    ...
    def check_SAT_Effect(self) -> bool:
            ...
            if isinstance(self.trace, ts.TimestampTrace):
                cf_trace_template = self.trace.cf_automaton(ts.TimestampCause([],[]), self.system.all_actions, False) 
                cf_trace_template.sat_check()           
            else: 
                cf_trace_template = self.trace.cf_automaton(ts.DelayCause([],[]), self.system.all_actions, False) 
            
            if isinstance(self.trace, ts.DelayTrace):
                cf_trace_template.sat_check()
